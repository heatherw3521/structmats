function H = hss_nudftv2(nodes, a, b,N, varargin)
...
istoep = 0; 
if ~isempty(varargin)
    typecheck = find(strcmp('toep', varargin));
    if ~isempty(typecheck)
        istoep = 1;   
    end
end 
...
H = BuildHSS_iter(H, tol, 0, 0,N, nodes, a, b', istoep);
end
...
%% SUBROUTINES
% 
function H = BuildHSS_iter(H, tol, mh, nh,N,nodes, a, b, istoep)
...
if H.leafnode
    ...
    % Call the constructor recursively on the left and right children.
      H.A11 = BuildHSS_iter(H.A11, tol, mh, nh, N, nodes, a, b, istoep);
    
      [mm1, nn] = size(H.A11); 
      [mm2, ~] = size(H.A22);
      mh = mh + mm1; 
      nh = nh + nn; 
      H.A22 = BuildHSS_iter(H.A22,tol, mh,nh,N, nodes, a, b, istoep);
