function [ZZ, DD, YY, all_res, X0] = fadi(A,B,U,V,varargin)
%factored ADI:
%
...
if compute_shifts == 1
    ...
    a = eigs(A, 1, 'SM'); % smallest magnitude
    b = eigs(A, 1, 'LM'); % largest magnitude
    c = eigs(B, 1, 'SM'); 
    d = eigs(B, 1, 'LM'); 
...
for i = 1:Ns-1 
    ...
    % residual evaluation at each step (inefficient, currently)
    % X = ZZ * diag(DD) * YY'; % approximate solution from fADI
    % all_res(i+1) = norm(A*X - X*B - U*V', "fro") / beta;
    % all_res(i+1) = norm(X-XTrue, "fro") / beta;
end
    % DD = diag(DD); 
end
