...
[T, Tinv, gam, cr] = mobiusT(I); 

%if tol is specified, find N: (need to adjust tol for this
% setting!)
if numel(varargin) > 1
    tol= varargin{2};
...
%set a guard parameter: 
if ~numel(varargin)==1
    guard = 4; 
    N = N + guard; %this adds a few extra fADI iterations
end            % for additional accuracy. 
               % This can be useful if, for instance, 
               % the norms of the submatrices are expected to be
               % large or the tolerance setting is 
               % large. 
