function [Z,rows,varargout] = inter_decomp(A,options)
...
elseif strcmp(side,'rows')
    [Zt, rows] = inter_decomp(A',ctype = cutofftype,cval = cutoffval,orientation = 'columns');
    Z = Zt';
elseif strcmp(side,'double-sided')
    [Z, rows] = inter_decomp(A,ctype = cutofftype,cval = cutoffval,orientation ='rows');
    [Z2,cols] = inter_decomp(A(rows,:),ctype = cutofftype,cval = cutoffval,orientation ='columns');
